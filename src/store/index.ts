...
import { variableProxy, subscriberProxy, subscribers, Config, configProxy } from './proxy'
...
export type Store = {
  ...
  map: Map<string, any>
  subscribers: Map<string, ((newValue?: any) => {})[]>
...
const store = create<Store>((set: SetState<Store>, get: GetState<Store>) => ({
  ...
  mapSubscribe: (
    ...
    // TODO: translate the shorthands based on the context (grid or cell)
    // we have to do this now because the callback list in the store is contextless

    const { subscribers } = get()

    const callbacksAlreadyPresent = subscribers.get(key)
    let newCallbacks = []
    if (Array.isArray(callbacksAlreadyPresent)) {
      newCallbacks = [...callbacksAlreadyPresent, callback]
    } else {
      newCallbacks = [callback]
    }
    const newSubscribers = new Map(subscribers).set(key, newCallbacks)
